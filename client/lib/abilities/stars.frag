...
const float period = 50000000.0;

void main() {
  float magnitude = 2.0;
  vec4 spectrum = vec4(10.0, 5.0, 1.0, 10.0);
  spectrum *= vec4(
    mix(0.8, 1.1, fractalNoise1(time / 500000.0)),
    mix(0.8, 1.0, fractalNoise1(time / 480000.0)),
    mix(0.9, 1.0, fractalNoise1(time / 2000000.0)),
    1.0
  );
  ...
  vec4 heat = vec4(0.9, 0.1, 0.1, 1.0) * smoothstep(0.0, 1.0, 2.0 * heatMask * noise2(vec2(time / 100000.0, sin((distanceFromCenter - 1.0) * pi) * noise), vec2(0, 0.0), vec2(1000.0, 1.0), 80.0)) * heatMask;
