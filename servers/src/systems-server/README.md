...
```bnf
<featuredata>       ::= <energyname> <energyunits> <disabled> <designmax> <currentmax> <currentused>
<energyname>        ::= <string>
<energyunits>       ::= <string>
<designmax>         ::= <double>
<currentmax>        ::= <double>
...
units `<energyunits>`. "Energy" is a broad concept, and clients should
avoid baking in assumptions about how energies work.

The `<designmax>` value is the amount of energy that the generator
would generate in ideal conditions. If this value is zero, the dynasty
does not have internals access to this asset.

The `<currentmax>` value is the amount of energy that the generator is
capable of generating in current conditions. It is less than or equal
to `<designmax>`.

The `<currentused>` is the amount of energy being consumed, of the
energy generated by this feature.


#### `fcEnergyConsumer` (0x23)

```bnf
<featuredata>       ::= <energyname> <energyunits> <disabled> <designmax> <currentused>
<energyname>        ::= <string>
<energyunits>       ::= <string>
<designmax>         ::= <double>
<currentused>       ::= <double>
...
The energy being consumed is named by `<energyname>`, which uses
units `<energyunits>`. "Energy" is a broad concept, and clients should
avoid baking in assumptions about how energies work.

The `<designmax>` value is the amount of energy that the consumer
would consume in ideal conditions. If this value is zero, the dynasty
does not have internals access to this asset.

The `<currentused>` is the amount of energy being consumed.
