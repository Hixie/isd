...
factory iterations. If the factory is stalled (see below), this is
zero.
 ...
 * `set-rate`: One field, a double. Sets the `<configuredrate>`. The
   ...
   contains no additional fields. If the factory is currently stalled,
   this re-enables the factory.

A factory completely shuts down (stalls) automatically if it cannot
sustain its `<configuredrate>` (due to insufficient inputs or
insufficient space for outputs); it does not automatically throttle
itself. This is reported in the `<disabled>` flags as either
`drCannotGuaranteeInput` or `drCannotStoreOutput`, and by the
`<currentrate>` being zero. To re-enable the factory, call `set-rate`
(possibly with the current `<configuredrate>`).
...
```bnf
<featuredata>       ::= <energyname> <energydesc> <energyunits> <disabled> <designmax> <currentmax> <currentused>
<energyname>        ::= <string>
<energydesc>        ::= <string>
<energyunits>       ::= <string>
<designmax>         ::= <double>
...
An energy-generation feature.

The energy being generated is named by `<energyname>`, which uses
units `<energyunits>` and has a description `<energydesc>`. "Energy"
is a broad concept, and clients should avoid baking in assumptions
about how energies work.

The `<designmax>` value is the amount of energy per millisecond that
the generator would generate in ideal conditions. If this value is
zero, the dynasty does not have internals access to this asset.

The `<currentmax>` value is the amount of energy that the generator is
capable of generating per millisecond in current conditions. It is
less than or equal to `<designmax>`.

The `<currentused>` is the amount of energy being consumed per
millisecond, of the energy generated by this feature.


#### `fcEnergyConsumer` (0x23)

```bnf
<featuredata>       ::= <energyname> <energydesc> <energyunits> <disabled> <designmax> <currentused>
<energyname>        ::= <string>
<energydesc>        ::= <string>
<energyunits>       ::= <string>
<designmax>         ::= <double>
...
An energy-consuming feature. Affects other features on the asset.

The energy being consumed is named by `<energyname>`, which uses units
`<energyunits>` and has a description `<energydesc>`. "Energy" is a
broad concept, and clients should avoid baking in assumptions about
how energies work.

The `<designmax>` value is the amount of energy that the consumer
would consume per millisecond in ideal conditions. If this value is
zero, the dynasty does not have internals access to this asset.

The `<currentused>` is the amount of energy being consumed by this
feature per millisecond.

The difference between the `<designmax>` and the `<currentused>` is
how effective the asset will be as a result of the energy shortage, if
any.
...
are defined as follows.
    ...
    0 (LSB) : `drNoBus`: The asset could not find a coordinating asset.
              For example, `fcMining` and `fcRefining` features need an
    ...
    1       : `drStructuralIntegrity`: The asset's structural integrity
              has not yet reached the minimum functional threshold (see
              `fcStructure` feature). Currently, this always entirely
              disables the feature.

    2       : `drManuallyDisabled`: The asset was manually disabled
              (see `fcOnOff` feature). This always entirely disables
              the feature.

    3       : `drUnderstaffed`: The number of staff assigned to the
              asset is below the required number (see `fcStaffing`
              feature). This rate-limits the feature in proportion to
              the fraction of jobs staffed.

    4       : `drUnowned`: The asset requires a dynasty to own it, and
              does not have one. This always entirely disables the
              feature.

    5       : `drCannotGuaranteeInput`: Meaning varies based on the
              feature, as follows:

              `fcFactory`: The asset is a factory configured to run at
    ...
    6       : `drCannotStoreOutput`: Meaning varies based on the
              feature, as follows:

              `fcFactory`: The asset is a factory whose output has
    ...
    7       : `drInsufficientEnergy`: There is insufficient energy for
              the asset function. The asset may be operating at reduced
              efficiency, rather than be entirely stopped.

    8       : `drConfiguration`: The factory is configured to run at
              a reduced rate, so anything depending on the factory is
              also going to be running at a reduced rate.
